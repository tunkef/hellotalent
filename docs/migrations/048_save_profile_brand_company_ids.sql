-- ═══════════════════════════════════════════════════════════════
-- HELLOTALENT — Migration 048: save_candidate_profile — brand/company IDs
-- Date: 2026-03-20
-- Type: RPC UPDATE — extends experience + brand_interest inserts with FK ids
--
-- Changes from 044:
--   1. candidate_experiences INSERT: added company_id, brand_id columns
--   2. candidate_brand_interests INSERT: added brand_id column
--   All text columns (sirket, marka) preserved — backward compatible.
--   Frontend sends ids when available; null for freeform/unresolved entries.
--
-- Depends on: 047 (ensures company_id/brand_id columns exist)
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- 048 CHANGE: added company_id and brand_id (nullable, backward compatible)
  ...
  -- 048 CHANGE: added brand_id (nullable, backward compatible)
