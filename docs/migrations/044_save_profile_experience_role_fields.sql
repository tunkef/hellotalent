-- ═══════════════════════════════════════════════════════════════
-- HELLOTALENT — Migration 044: Fix experience role field persistence
-- Date: 2026-03-20
-- Type: BUGFIX — fixes silent data loss
--
-- Problem:
--   save_candidate_profile() INSERT for candidate_experiences does
--   NOT include rol_ailesi or rol_unvani. The front-end sends both
--   fields (profil-ui.js collectExperiences lines 836-837) but the
--   RPC silently drops them. Worse: because the RPC does
--   DELETE + re-INSERT on every save, any manually-entered values
--   are destroyed on each profile save.
--
-- Fix:
--   Add rol_ailesi and rol_unvani to the experience INSERT statement.
--   No other changes to the RPC logic.
--
-- Depends on: 043 (ensures columns exist on candidate_experiences)
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- *** FIX: added rol_ailesi and rol_unvani (were missing, causing silent data loss) ***
