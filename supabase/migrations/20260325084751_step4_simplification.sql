-- ═══════════════════════════════════════════════════════════════
-- Migration: Step 4 simplification — target-role constraint, career_type normalization,
--            brand-interest auto-follow
-- Date: 2026-03-25
--
-- 1. Drop stale candidate_target_roles_rol_ailesi_check (001 whitelist
--    is out of sync with current frontend SEKTOR_ROL_MAP taxonomy).
--    Replace with simple NOT NULL + non-empty text check.
--
-- 2. Normalize legacy career_type values:
--    - any value containing 'yukari' or 'lider' → 'yukari'
--    - pure 'yatay' → 'yatay'
--    - ambiguous multi-value → 'yukari'
--    Then add CHECK for new single-select contract.
--
-- 3. Extend save_candidate_profile: after brand_interests INSERT,
--    also INSERT into candidate_brand_follows for any brand_id present.
--    Uses ON CONFLICT DO NOTHING (idempotent, preserves manual follows).
--
-- Depends on: Phase 2 migration (latest save_candidate_profile version)
-- Safe: CHECK drop is non-destructive, career_type normalization is one-shot,
--       brand-follow INSERT is additive only.
-- ═══════════════════════════════════════════════════════════════


-- ════════════════════════════════════════════
-- PART 1: Drop stale rol_ailesi CHECK constraint
-- ════════════════════════════════════════════

-- The constraint name from 001 migration
ALTER TABLE candidate_target_roles
  DROP CONSTRAINT IF EXISTS candidate_target_roles_rol_ailesi_check;

-- Keep rol_ailesi as required non-empty text (no whitelist)
-- The NOT NULL already exists from CREATE TABLE; just add non-empty guard
ALTER TABLE candidate_target_roles
  DROP CONSTRAINT IF EXISTS candidate_target_roles_rol_ailesi_nonempty;
ALTER TABLE candidate_target_roles
  ADD CONSTRAINT candidate_target_roles_rol_ailesi_nonempty
  CHECK (trim(rol_ailesi) <> '');


-- ════════════════════════════════════════════
-- PART 2: Normalize legacy career_type values
-- ════════════════════════════════════════════

-- Step A: normalize existing data
UPDATE candidate_work_preferences
SET career_type = CASE
  WHEN career_type IS NULL THEN NULL
  WHEN career_type LIKE '%yukari%' THEN 'yukari'
  WHEN career_type LIKE '%lider%' THEN 'yukari'
  WHEN career_type = 'yatay' THEN 'yatay'
  -- ambiguous multi-value (e.g. 'yatay,lider') → yukari
  WHEN career_type LIKE '%,%' THEN 'yukari'
  ELSE career_type
END
WHERE career_type IS NOT NULL;

-- Step B: add single-select CHECK
...
ALTER TABLE candidate_work_preferences
  ...
  CHECK (career_type IS NULL OR career_type IN ('yukari', 'yatay'));


-- ════════════════════════════════════════════
-- PART 3: Extend save_candidate_profile with brand-follow sync
-- ════════════════════════════════════════════
...
BEGIN
  ...
  -- Sync brand interests with resolved brand_id into candidate_brand_follows.
  -- Uses ON CONFLICT DO NOTHING to preserve existing manual follows.
