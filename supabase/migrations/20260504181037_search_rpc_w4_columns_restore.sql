-- ═══════════════════════════════════════════════════════════════════
-- Migration: search_rpc_w4_columns_restore
-- Date: 2026-05-04
-- Ref: BUG 2 hotfix — search_employer_candidates RPC kırık
-- Tier: T3 (RPC fix, runtime exception düzeltir)
-- ─────────────────────────────────────────────────────────────────
-- KRİTİK BUG:
--   `search_employer_candidates` RPC'si runtime'da
--   `column wc.travel_willingness does not exist` (SQLSTATE 42703)
--   atıyordu. PostgREST 500 dönüyordu, JS catch boş array,
--   UI 0 aday + Pozisyon dropdown empty + spinner stuck.
--
-- HISTORY:
--   - 2026-04-30 (20260430211728_search_rpc_candidate_id_filter)
--     with_children CTE'sinde travel_willingness/shift_flexibility/
--     notice_period EKSİKTİ (final SELECT bunları çağırıyordu).
--     RPC zaten broken'dı.
--   - 2026-05-01 (20260501145426_test_seed_admin_bypass) içinde
--     "W4 FIX" yorumlu satırlar 3 kolonu with_children'a ekledi.
--     RPC çalışmaya başladı.
--   - 2026-05-03 (20260503194552_search_candidates_lifecycle_guard)
--     CREATE OR REPLACE ile RPC'yi yeniden tanımladı, baz aldığı
--     20260430211728 olduğu için W4 FIX'i KOPYALAMADI. RPC tekrar
--     kırıldı. Lint runtime'da yakalandı, UAT mock olduğu için kaçırdı.
--
-- TESPİT:
--   `supabase db lint --linked` 2026-05-04 çalıştırılırken görüldü.
--   pool 0/0 + Pozisyon dropdown empty Tuna ekranında stuck spinner.
--
-- BU MIGRATION:
--   Lifecycle guard'ı koruyarak (A8-B Task 3) iki overload'a
--   travel_willingness / shift_flexibility / notice_period kolonlarını
--   with_children CTE'sinde restore eder. Eski 20260503194552 dosyası
--   da repo'da düzeltildi (canonical kaynak doğru olsun).
--
-- VALIDATION:
--   Apply sonrası `supabase db lint --linked` "column does not exist"
--   hatası kaybolmalı. Tuna canlı'da hr-pool refresh → 205 aday görmeli.
--
-- ROLLBACK:
--   20260503194552 migration SQL'ini tekrar CREATE OR REPLACE ile
--   uygula (W4 FIX olmadan). Bu kullanıcıyı tekrar broken state'e
--   döndürür → ÖNERİLMEZ.
--
...
-- ─────────────────────────────────────────────────────────────────
-- 6-PARAM OVERLOAD — lifecycle guard + W4 FIX restore
-- Body: 20260503194552 baz alınarak with_children CTE düzeltildi
-- ─────────────────────────────────────────────────────────────────
  ...
  SELECT jsonb_build_object(
    ...
    'candidates', COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        ...
        -- telefon/email employer havuzunda DÖNDÜRÜLMEZ.
...
-- ─────────────────────────────────────────────────────────────────
-- 5-PARAM OVERLOAD — lifecycle guard + W4 FIX restore
-- ─────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION search_employer_candidates(
  p_filters jsonb DEFAULT '{}'::jsonb,
  p_employer_company_id bigint DEFAULT NULL,
...
-- Grants korunur (önceki migrations'dan miras)
COMMENT ON FUNCTION search_employer_candidates(jsonb, bigint, text, int, int, bigint) IS
  'A8-B Task 3 lifecycle guard + W4 FIX restore (2026-05-04). 6-param canonical.';
COMMENT ON FUNCTION search_employer_candidates(jsonb, bigint, text, int, int) IS
  'A8-B Task 3 lifecycle guard + W4 FIX restore (2026-05-04). 5-param compat.';
