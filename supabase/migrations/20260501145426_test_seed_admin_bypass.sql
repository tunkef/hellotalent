-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║  Migration: test_seed_admin_bypass                                     ║
-- ║  Tarih: 2026-05-01                                                     ║
-- ║  Yazar: supabase-agent (Phase H.B admin bypass)                        ║
-- ╠══════════════════════════════════════════════════════════════════════════╣
-- ║  Amaç:                                                                 ║
-- ║    H.B seed adaylarını (test_seed_batch IS NOT NULL) admin employer    ║
-- ║    hesaplarına görünür kılmak — UAT için Tuna pool'da fake adayları    ║
-- ║    görebilsin. recruiter/viewer erişemez (production safety korunur).  ║
-- ║                                                                        ║
-- ║  Kapsam:                                                               ║
-- ║    1. is_admin_employer() helper fn (SECURITY DEFINER + hardened)     ║
-- ║    2. candidates_employer_read policy — admin bypass eklendi           ║
-- ║    3. _search_employer_candidates_internal 6-param — visible CTE fix  ║
-- ║    4. search_employer_candidates 5-param — visible CTE fix            ║
-- ║                                                                        ║
-- ║  KVKK / Güvenlik Notu:                                                ║
-- ║    Seed adayların verileri test data'dır; PII içermez (scripts/       ║
-- ║    seed-test-candidates.js fake isim/şehir oluşturur).                ║
-- ║    PII strip (W3) korunur: admin dahil telefon/email dönmez.          ║
-- ║    recruiter/viewer bypass yok — sadece employer_role='admin'.        ║
-- ║                                                                        ║
-- ║  6-param _internal body: birebir 20260501134009 apply sonrası live    ║
-- ║    prosrc — sadece visible CTE WHERE'deki 1 satır değişti:            ║
-- ║    AND c.test_seed_batch IS NULL                                       ║
-- ║    → AND (c.test_seed_batch IS NULL OR is_admin_employer())           ║
-- ║                                                                        ║
-- ║  5-param body: aynı tek satır değişikliği.                            ║
-- ║                                                                        ║
-- ║  APPLY YOK — T3 tier: auditor + code-reviewer + Codex + Tuna onayı. ║
-- ╚══════════════════════════════════════════════════════════════════════════╝

-- ═══════════════════════════════════════════════════════════
-- 1. HELPER: is_admin_employer()
-- SECURITY DEFINER: hr_profiles'a RLS bypass olmadan erişir.
-- search_path hardened (A14 pattern).
-- REVOKE PUBLIC + GRANT authenticated.
-- ═══════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION is_admin_employer()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION is_admin_employer() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION is_admin_employer() TO authenticated;

COMMENT ON FUNCTION is_admin_employer() IS
  'Phase H.B admin bypass — employer_role=admin ise true döner. '
  'UAT: admin test_seed_batch fake adaylarını görebilir. '
  'recruiter/viewer için false (production safety korunur). '
  'SECURITY DEFINER: hr_profiles RLS bypass gerektirmez, auth.uid() yeterli.';

-- ═══════════════════════════════════════════════════════════
-- 2. candidates_employer_read POLICY UPDATE
-- Mevcut USING (live verify 2026-05-01):
--   is_employer()
--   AND is_active = true
--   AND (profile_completed = true OR profile_completion_pct >= 45)
--   AND test_seed_batch IS NULL
--
-- Yeni: test_seed_batch IS NULL → (test_seed_batch IS NULL OR is_admin_employer())
-- Diğer koşullar birebir korundu.
...
CREATE POLICY candidates_employer_read ON candidates
  ...
  USING (
    ...
    AND is_active = true
    AND (profile_completed = true OR profile_completion_pct >= 45)
    AND (test_seed_batch IS NULL OR is_admin_employer())
  );

-- ═══════════════════════════════════════════════════════════════════════════
-- 3. _search_employer_candidates_internal (6-param) — visible CTE fix
--
-- Temel: 20260501134009 apply sonrası canlı prosrc (birebir kopya).
-- Sadece visible CTE WHERE'de 1 satır değişti:
--   AND c.test_seed_batch IS NULL
--   → AND (c.test_seed_batch IS NULL OR is_admin_employer())
-- Geri kalan her satır, yorum ve whitespace korundu.
--
-- SECURITY: REVOKE PUBLIC; authenticated direkt GRANT yok (wrapper-only).
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  visible AS (
    ...
    WHERE c.is_active = true
      ...
      AND (c.test_seed_batch IS NULL OR is_admin_employer())   -- H.B admin bypass
...
--   AND c.test_seed_batch IS NULL
--   → AND (c.test_seed_batch IS NULL OR is_admin_employer())
-- W3 INLINE PII STRIP (telefon/email yok) korundu.
-- SECURITY DEFINER + search_path + REVOKE/GRANT aynen.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- 1. Helper:
--    DROP FUNCTION IF EXISTS is_admin_employer();
--
-- 2. Policy geri al (H.B öncesi hali):
--    DROP POLICY IF EXISTS candidates_employer_read ON candidates;
--    CREATE POLICY candidates_employer_read ON candidates
...
--      );
--
-- 3. _internal 6-param: 20260501134009 body'yi CREATE OR REPLACE ile geri yükle.
-- 4. 5-param: 20260501134009 body'yi CREATE OR REPLACE ile geri yükle.
-- Not: Rollback = seed adaylar admin'e de görünmez (H.B öncesi production safety).
-- ═══════════════════════════════════════════════════════════
