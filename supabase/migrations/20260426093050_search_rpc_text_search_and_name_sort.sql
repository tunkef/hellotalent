-- ═══════════════════════════════════════════════════════════════
-- Migration: search_employer_candidates — text search + name sort
-- Date: 2026-04-26
-- Based on: 20260404092358_search_rpc_new_fields.sql (Phase 3 / new fields)
-- CREATE OR REPLACE — safe overwrite, no schema change, no new tables.
--
-- Changes from previous version:
--   1. DECLARE: added f_search text variable
--   2. Filter extraction: f_search := NULLIF(trim(p_filters->>'search'), '')
--   3. filtered CTE: added text search WHERE clause
--      (full_name OR son_pozisyon OR adres_il ILIKE '%search%')
--      NULL-safe: f_search IS NULL short-circuits → backwards-compatible
--   4. sorted CTE: added CASE WHEN p_sort = 'name' THEN s.full_name END ASC
--      Collation: default PostgreSQL lexicographic (COLLATE "tr-x-icu" deferred — not confirmed on prod)
--   5. WAVE 3: wildcard escape applied to f_search (\ % _ → literals via v_search_escaped + ESCAPE '\')
--
-- Backwards compatibility:
--   - Function signature UNCHANGED (same params, same return type)
--   - Clients not sending 'search' in p_filters → f_search IS NULL → no-op
--   - Clients not using 'name' sort → CASE evaluates NULL → skipped
--   - All existing sort values (relevance, newest, exp_asc, exp_desc) intact
--
-- Performance note:
--   ILIKE full-scan acceptable at current scale (6 live candidates, <200 seeded).
--   For scale: pg_trgm GIN index on full_name + son_pozisyon — backlog Wave 3.
--
-- No new tables. No RLS changes. No grants required (SECURITY DEFINER retained).
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  filtered AS (
    ...
    FROM with_langs wl
    WHERE
      -- WAVE 3: text search — full_name + son_pozisyon + adres_il
      -- NULL-safe: f_search IS NULL → clause skipped (backwards-compatible)
      -- v_search_escaped: \, %, _ chars escaped → ILIKE treats them as literals (no DoS, no wildcard bleed)
      (f_search IS NULL
  ...
  sorted AS (
    ...
    ORDER BY
      ...
      CASE WHEN p_sort = 'name'         THEN s.full_name END ASC NULLS LAST,  -- YENI: alfabetik ad sortu
...
-- Test scenarios (Wave 3 wildcard escape):
--   f_search = 'Ahmet_Yılmaz' → '_' escaped to '\_', treated as literal underscore, not any-char wildcard
--   f_search = '%_%_%_%'      → all chars escaped, no ILIKE amplification, no DoS CPU spike
--   f_search = 'C:\Users'     → '\' escaped to '\\', no broken ESCAPE sequence
--   f_search = 'Mehmet'       → normal search, behaves identically to pre-Wave-3 (no special chars)
--   f_search = NULL or ''     → v_search_escaped stays NULL, WHERE clause short-circuits via f_search IS NULL
