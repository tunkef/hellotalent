-- ═══════════════════════════════════════════════════════════════
-- Migration: Supabase Advisor Fixes (SA1 + SA3 + SA5)
-- Date: 2026-04-06
--
-- SA1: Add SET search_path = public to 8 pre-baseline functions
--      that were missing it (Security Advisor warning).
-- SA2: SKIPPED — candidate_view_stats table does not exist.
-- SA3: Add indexes on 7 unindexed foreign key columns
--      (Performance Advisor info).
-- SA5: Add bio field to search_employer_candidates RPC output
--      so employers can see candidate bios in search results.
-- ═══════════════════════════════════════════════════════════════

-- ┌─────────────────────────────────────────────────────────────┐
-- │ SA1: Fix search_path for pre-baseline functions             │
-- └─────────────────────────────────────────────────────────────┘

-- Use DO blocks so missing functions don't abort the migration.
-- Some pre-baseline functions may have been replaced or dropped.
DO $$ BEGIN
  ALTER FUNCTION set_candidates_updated_at() SET search_path = public;
EXCEPTION WHEN undefined_function THEN NULL;
END $$;

DO $$ BEGIN
  ALTER FUNCTION set_companies_updated_at() SET search_path = public;
EXCEPTION WHEN undefined_function THEN NULL;
END $$;

DO $$ BEGIN
  ALTER FUNCTION update_candidate_competencies_updated_at() SET search_path = public;
EXCEPTION WHEN undefined_function THEN NULL;
END $$;

DO $$ BEGIN
  ALTER FUNCTION update_candidate_view_stats() SET search_path = public;
EXCEPTION WHEN undefined_function THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER FUNCTION update_unread_inbox_count() SET search_path = public;
EXCEPTION WHEN undefined_function THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER FUNCTION is_admin_role(text) SET search_path = public;
EXCEPTION WHEN undefined_function THEN NULL;
END $$;

-- ┌─────────────────────────────────────────────────────────────┐
-- │ SA3: Add missing FK indexes                                 │
-- └─────────────────────────────────────────────────────────────┘

-- Safe index creation: skip if table doesn't exist in production.
DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_campaign_clicks_candidate
    ON campaign_clicks (candidate_id);
EXCEPTION WHEN undefined_table THEN NULL;
END $$;

DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_campaign_redemptions_candidate
    ON campaign_redemptions (candidate_id);
EXCEPTION WHEN undefined_table THEN NULL;
END $$;

DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_campaign_reviews_reviewer
    ON campaign_reviews (reviewer_id);
EXCEPTION WHEN undefined_table THEN NULL;
END $$;

DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_inbox_messages_sender
    ON inbox_messages (sender_user_id);
EXCEPTION WHEN undefined_table THEN NULL;
END $$;

DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_inbox_messages_company
    ON inbox_messages (company_id);
EXCEPTION WHEN undefined_table THEN NULL;
END $$;

DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_inbox_messages_brand
    ON inbox_messages (brand_id);
EXCEPTION WHEN undefined_table THEN NULL;
END $$;
...
-- ┌─────────────────────────────────────────────────────────────┐
-- │ SA5: Add bio to search_employer_candidates RPC              │
-- │      CREATE OR REPLACE — safe overwrite of existing RPC.    │
-- │      Based on: 20260404092358_search_rpc_new_fields.sql     │
-- │      Only change: bio added to visible CTE, with_children,  │
-- │      and final jsonb output.                                │
-- └─────────────────────────────────────────────────────────────┘
