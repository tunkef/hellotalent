-- ═══════════════════════════════════════════════════════════════
-- K-066 — Split Wizard + Live CV Preview (Template-Driven, AI-Free)
-- 1) AI CV feature kolonunu kaldır (artık Anthropic çağrısı yok,
--    template-driven deterministic CV render ediliyor)
-- 2) candidate_work_preferences'a career_mobility kolonu ekle
--    (İK filtresi: yatay/dikey geçiş niyeti — CV'de görünmez)
-- 3) save_candidate_profile RPC'sine career_mobility ekle
-- ═══════════════════════════════════════════════════════════════

-- 1. AI CV kullanım kolonunu düşür (ai_assessment_used KORUNUR — ayrı feature)
ALTER TABLE public.candidates
  DROP COLUMN IF EXISTS ai_cv_used;

-- 2. career_mobility kolonu — work preferences child table
ALTER TABLE public.candidate_work_preferences
  ADD COLUMN IF NOT EXISTS career_mobility text
  CHECK (career_mobility IS NULL OR career_mobility IN ('vertical','horizontal','both','none'));

COMMENT ON COLUMN public.candidate_work_preferences.career_mobility IS
  'Kariyer geçiş niyeti (İK filtresi, CV''de görünmez): vertical=dikey, horizontal=yatay, both=ikisi de, none=değişim istemiyor';

-- 3. save_candidate_profile RPC güncelle — career_mobility dahil
...
BEGIN
  ...
  -- ── WORK PREFERENCES (+ career_mobility — K-066) ──────────
  IF p_work_prefs IS NOT NULL THEN
    INSERT INTO candidate_work_preferences (
      ...
      travel_willingness, shift_flexibility, notice_period,
      career_mobility
    )
    VALUES (
      v_candidate_id,
      ...
      p_work_prefs->>'travel_willingness',
      p_work_prefs->>'shift_flexibility',
      p_work_prefs->>'notice_period',
      NULLIF(p_work_prefs->>'career_mobility', '')
    )
    ON CONFLICT (candidate_id) DO UPDATE SET
      ...
      shift_flexibility   = EXCLUDED.shift_flexibility,
      notice_period       = EXCLUDED.notice_period,
      career_mobility     = EXCLUDED.career_mobility;
  END IF;
...
-- cv_url, cv_filename, cv_uploaded_at kolonları KORUNUR (legacy CV upload opsiyonel kalır).
